jQuery(document).ready(function ($) {
    var geocoder, map, directionsDisplay, directionsService, geolocationLatlng, autoCompleteLatLng,
        ...
        mapDefaults             = {},
        retailersList           = wpslAjaxVariables.retailers,
        retailersGroups         = wpslAjaxVariables.retailers_groups,
        resetMap                = false,
        streetViewAvailable     = false,
        autoLoad                = ( typeof wpslSettings !== "undefined" ) ? wpslSettings.autoLoad : "",
        isDirectionsDisplayed = false,
...
// Only continue if a map is present.
    ...
    function initializeGmap(mapId, mapIndex) {
        var mapOptions, infoWindow, latLng,
        ...
        if ($("#wpsl-gmap").length) {
            ...
            if (!$(".wpsl-search").hasClass("wpsl-widget") && !$("#wpsl-search-input").val()) {
    ...
    /**
     * Activate the autocomplete for the store search and add retailers.
     *
     * @since 2.2.0
     * @link https://developers.google.com/maps/documentation/javascript/places-autocomplete
     * @returns {void}
     */
    function checkRetailers(e) {
        if ((e.which <= 90 && e.which >= 48) || e.which == 32 || e.which == 8) {
            $(".pac-item.custom-item").each(function() {
                $(this).remove();
            });
            ...
            var retailers = [];
            $('input[name="retailer"]:checked').each(function () {
            ...
            var sub_ranges = [];
            $('input[name="sub_range"]:checked').each(function () {
                sub_ranges.push($(this).val());
            });

            if (retailersList.length > 0) {
                retailersList.forEach(function(item) {
                    var lowerTtitle = item.title.toLowerCase();
                    if (lowerTtitle.indexOf(lowerValue) !== -1 && count < 5) {
                        if (retailers.length < 1 || retailers.indexOf(item.retailer_id) !== -1) {
                            if (ranges.length < 1 || commonElements(ranges, item.ranges)) {

                                if (sub_ranges.length < 1 || commonElements(sub_ranges, item.subranges)) {
                                    var title = item.title;
                                    $(".pac-container").append('<div class="pac-item custom-item" data-item-id="'+item.id+'" data-item-lat="'+ item.lat +'" data-item-lng="'+ item.lng +'"><span class="_retailer-mark">R</span><span class="pac-item-query">'+ highlightText(value, title)+'</span><span>'+ item.city + ', ' + item.address +'</span></div>');
                                    count++;
                                }
                            }
                        }
    ...
    function highlightText(text, title)
    {
        ...
        var index = titleLower.indexOf(textLower);
        if ( index >= 0 )
        { 
            title = title.substring(0,index) + '<span class="pac-matched">' + title.substring(index,index+text.length) + '</span>' + title.substring(index + text.length);
        }
        return title;
    }
        ...
        function addEventListenerWrapper(type, listener) {
            ...
            if (type == "keydown") {
                ...
                listener = function(event) {
                    ...
                    var custom_items = $(".custom-item").length > 0;
                    var custom_item_selected = $(".custom-item.pac-item-selected").length > 0;
                    var selectedCustom = $(".custom-item.pac-item-selected");
                    if (event.which == 13 && !suggestion_selected) {
                        var firstEl = $(".pac-item").first();
                        var retailerGroupActive = false;
                        if (retailersGroups.length > 0 && input.value !== '') {
                            retailersGroups.forEach(function(item) {
                                if (!retailerGroupActive) {
                                    var RetailerTitle = item.title;
                                    var lowerTtitle = item.title.toLowerCase();
                                    var lowerValue = input.value.toLowerCase();
                                    lowerTtitle = lowerTtitle.replace(/'/g, "");
                                    lowerTtitle = lowerTtitle.replace(/`/g, "");
                                    lowerValue = lowerValue.replace(/'/g, "");
                                    lowerValue = lowerValue.replace(/`/g, "");
                                    if (lowerTtitle.indexOf(lowerValue) !== -1) {
                                        ...
                                        $('input[name="retailer"][value='+ item.id + ']').prop('checked', true);
                                        $('input[name="retailer"][value='+ item.id + ']').parent().addClass('active');
                                        input.value = RetailerTitle;
                                        var settings = getMapSettings(0);
                                        infoWindow = newInfoWindow();
                                        showStores(settings.startLatLng, infoWindow);
                                        retailerGroupActive = true;
                        ...
                        if (!retailerGroupActive) {
                            if (firstEl.hasClass('custom-item')) {
                                firstEl.trigger("touchstart");
                                $(".pac-container").css("display","none");
                            } else {
                    ...
                    } else if (event.which == 13 && custom_item_selected) {
                        selectedCustom.trigger("touchstart");
                        $(".pac-container").css("display","none");
                    } else if (event.which == 38 && custom_items) {
                        if (!suggestion_selected || $(".pac-item.custom-item").first().hasClass('pac-item-selected')) {
                            orig_listener.apply(input, [event]);
                            $(".pac-item.custom-item").removeClass('pac-item-selected');
                        } else if (custom_item_selected || $(".pac-item-selected").prev().hasClass('custom-item')) {
                            $(".pac-container").show();
                            var selected = $(".pac-item-selected");
                            selected.prev().addClass('pac-item-selected');
                            selected.removeClass('pac-item-selected');
                            setTimeout(function(){
                                input.value = selected.prev().find('.pac-item-query').text();
                            });
                        } else  {
                            orig_listener.apply(input, [event]);
                        }
                    } else if (event.which == 40 && custom_items) {
                        if (!suggestion_selected ) {
                            $(".pac-container").show();
                            $(".custom-item").first().addClass('pac-item-selected');
                            input.value = $(".custom-item").first().find('.pac-item-query').text();
                        } else if (custom_item_selected && selectedCustom.next().hasClass('custom-item')){
                            $(".pac-container").show();
                            selectedCustom.next().addClass('pac-item-selected');
                            selectedCustom.removeClass('pac-item-selected');
                            input.value = selectedCustom.next().find('.pac-item-query').text();
                        } else {
                            $(".custom-item.pac-item-selected").removeClass('pac-item-selected');
                            orig_listener.apply(input, [event]);
                        }
                    } else {
                        orig_listener.apply(input, [event]);
    ...
    function activateAutocomplete() {
        ...
        input.addEventListener("keyup", checkRetailers);
...
// Handle the click on the back button when the route directions are displayed.
    ...
    var detailsBounds = null;
    
    function showModalTwist(elem) {
        var modal = elem.next().clone();
        if ($(modal).hasClass('_twist-modal-popup')) {
    ...
    function offsetCenter(offsetx, offsety) {
        if (isMobile) {
            return;
        }
    ...
    function codeAddress(infoWindow) {
        ...
        if (!$("#wpsl-search-input").val()) {
            prepareStoreSearch(settings.startLatLng, infoWindow);
            return;
        }
    ...
    function makeAjaxRequest(startLatLng, resetMap, autoLoad, infoWindow) {
        ...
        $('.app-store-searchbox__result').text('');
        var searchBoxText = '';
        ...
        $.get(wpslSettings.ajaxurl, ajaxData, function (response) {
            ...
            if (response.length > 0) {
                searchBoxText = 'Showing the ' + response.length + ' closest retailers';
                if ($('#wpsl-search-input').val()) {
                    searchBoxText += ' to ' + $('#wpsl-search-input').val();
                }
                // $('.app-store-searchbox__result').text(
                //     'Showing the ' + response.length + ' closest retailers to ' + $('#wpsl-search-input').val() + ' selling Miracoil Advance mattresses'
                // );
                var rangesText = '';
                ...
                if (rangesText !== '') {
                    searchBoxText += ' selling ' + rangesText + ' mattresses';
                }
            ...
            $('.app-store-searchbox__result').text(searchBoxText);
     ...
     * @returns {void}
     */
    function makeAjaxRequestCustom(itemId, lat, lng) {
        var latLng, noResultsMsg, marker, len,
            ajaxData   = {},
            storeData  = "",
            draggable  = false,
            template   = $("#wpsl-listing-template").html(),
        ...
        ajaxData = {
            action: "store_search",
            max_results: wpslSettings.maxResults,
            search_radius: wpslSettings.searchRadius,
            lat: lat,
            lng: lng,
            item_id: itemId
        };
        ...
        ajaxData.retailers = retailers;

        var ranges = [];
        $('input[name="range"]:checked').each(function () {
            ranges.push($(this).val());
        });
        ajaxData.ranges = ranges;

        var sub_ranges = [];
        $('input[name="sub_range"]:checked').each(function () {
            sub_ranges.push($(this).val());
        });
        ajaxData.sub_ranges = sub_ranges;
        ...
        $.get(wpslSettings.ajaxurl, ajaxData, function (response) {
            ...
            if (response.length > 0) {
                searchBoxText = 'Showing the ' + $('#wpsl-search-input').val() + ' retailer';
                // $('.app-store-searchbox__result').text(
                //     'Showing the ' + response.length + ' closest retailers to ' + $('#wpsl-search-input').val() + ' selling Miracoil Advance mattresses'
                // );
                var rangesText = '';
                ...
                for (i = 0, len = markersArray.length; i < len; i++) {
                    if (markersArray[i].storeId == itemId) {
                        marker     = markersArray[i];
                        var bounds = new google.maps.LatLngBounds();
                        bounds.extend(marker.getPosition());
                        map.fitBounds(bounds);
                ...
                addMarker(settings.startLatLng, 0, '', true, infoWindow);
    ...
    function collectAjaxData(startLatLng, resetMap, autoLoad) {
        ...
        ajaxData.collections = collections;
    ...
    function addMarker(latLng, storeId, infoWindowData, draggable, infoWindow) {
        ...
        if (storeId === 0) {
            ...
            if (infoWindowData.offers) {
                url = "/wp-content/themes/raptor/img/markers/marker-fill.png";
            } else {
                url = "/wp-content/themes/raptor/img/markers/marker.png";
            }
        }
// debugger
        mapIcon = {
...
// Check if we are using both the infobox for the info windows and have marker clusters.
    ...
    $(document).on('change', 'input[name="retailer"]', function (e) {
    ...
    $(document).on('change', 'input[name="range"],input[name="sub_range"]', function (e) {
    ...
    $(document).on('change', 'input[name="collections"]', function (e) {
        $('input[name="range"],input[name="sub_range"]').each(function() {
            $(this).prop('checked', false);
            $(this).closest('label').removeClass('active');
        });
        var input        = document.getElementById("wpsl-search-input");
    ...
    $(document).on('click', '#mapView', function (e) {
        ...
        if (isDirectionsDisplayed) {
            return;
        }

        google.maps.event.trigger(map, 'resize');

        var $detailsBox = $('.app-filter-result__detail-box');
        if ($detailsBox.is('.opened') && detailsBounds) {
            map.fitBounds(detailsBounds);
            $detailsBox.hide();
        } else {
            fitBounds();
        }
    ...
    $(document).on('mousedown touchstart', '.pac-item.custom-item', function (e) {
        var element = $(e.target);
        if (!element.hasClass('custom-item')) {
            element = element.closest('.custom-item');
        }
        var text = element.find('.pac-item-query').text();
        
        $('#wpsl-search-input').val(text);
        
        var id = element.data('item-id');
        var lat = element.data('item-lat');
        var lng = element.data('item-lng');

        if (typeof id !== 'undefined' && id !== '') {
            makeAjaxRequestCustom(id, lat, lng);
        }
    });

    var listViewTab = document.getElementById("listView");
    listViewTab.onclick = function (e) {
        var e = e || window.event;
        var target = e.target || e.srcElement;
        $('div').removeClass('tabsDisplayBg displaysNoneTabs')
        $(".app-map-big").addClass("displaysNoneTabs")
        $(".app-filter-result__tabs__detail").addClass("tabsDisplayBg")
    }

    var mapViewTab = document.getElementById("mapView");
    mapViewTab.onclick = function (e) {
        var e = e || window.event;
        var target = e.target || e.srcElement;
        $('div').removeClass('tabsDisplayBg displaysNoneTabs')
        $(".app-filter-result__list-box").addClass("displaysNoneTabs")
