...
class GFGEO_Full_Address_Field extends GF_Field {
	...
	public function get_field_input( $form, $value = '', $entry = null ) {
		...
		return sprintf( "<div id='gfgeo-address-locator-wrapper-%s' class='ginput_container ginput_container_gfgeo_address nfgeo-address-locator-wrapper'>%s</div>", $form_id.'_'.$id, $input );
	}
	...
	public function get_value_merge_tag( $address, $input_id, $entry, $form, $modifier, $raw_value, $url_encode, $esc_html, $format, $nl2br ) {
		...
		if ( ! empty( $this->gfgeo_google_maps_link ) ) {
			...
			// link to google map
			$map_it = ' - <a class="gfgeo-address-map-it" href="'. esc_url( 'http://www.google.com/maps/place/'. str_replace( ' ', '+', $address ) ).'" target="_blank">'.__( 'View in Google Maps', 'gfgeo' ).'</a>'; 
		
		}
		
		$address = esc_html( $address ) . $map_it;
	...
	public function get_value_entry_list( $address, $entry, $field_id, $columns, $form ) {
		...
		$map_it = '<a style="font-size:11px;text-decoration: underline" class="gfgeo-address-map-it" href="'. esc_url( 'http://www.google.com/maps/place/'. str_replace( ' ', '+', $address ) ).'" target="_blank">'.__( 'View in Google Maps', 'gfgeo' ).'</a>'; 
		
		$address = '<span style="white-space: normal;margin-bottom:4px;">' .esc_html( $address ) . '</span><br /> ' . $map_it;
