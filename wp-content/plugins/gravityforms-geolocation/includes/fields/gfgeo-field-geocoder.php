...
class GFGEO_Geocoder_field extends GF_Field {
	 ...
	 * @return [type]                [description]
	 */
	public function get_geocoded_data_output( $geocoder_data ) {
		...
		if ( ! empty( $this->gfgeo_google_maps_link ) ) {
			...
			if ( ! empty( $geocoder_data['latitude'] ) && ! empty( $geocoder_data['longitude'] ) ) {
				$map_it .= '<a class="gfgeo-sub-address-map-it" href="'. esc_url( 'http://www.google.com/maps/place/'.$geocoder_data['latitude'].','.$geocoder_data['longitude'] ).'" target="_blank">'.__( 'View in Google Maps', 'gfgeo' ).'</a>'; 
			}
		}
		...
		$geocoder_data = array_combine( $default_geo_fields, $geocoder_data );
	...
	public function get_value_merge_tag( $geocoder_data, $input_id, $entry, $form, $modifier, $raw_value, $url_encode, $esc_html, $format, $nl2br ) {
		...
		// if passing value as a whole
			...
			if ( ! empty( $form['confirmation']['queryString'] ) ) {
				...
				return $this->get_geocoded_data_output( $raw_value );
			}
		}
	}
	...
	public function get_value_entry_detail( $geocoder_data, $currency = '', $use_text = false, $format = 'html', $media = 'screen' ) {
		...
		if ( empty( $geocoder_data ) || $format == 'text' ) {
			return $geocoder_data;
		}

		return $this->get_geocoded_data_output( $geocoder_data );
	}
