...
class GFMarketo {
    ...
    private static $version = "1.3.7.4";
    ...
    public static function init(){
        ...
        if(self::is_marketo_page()){
            ...
            // add_action( 'gform_after_submission', array( 'GFMarketo', 'export'), 10, 2); // Old SOAP method

            // New REST method
            add_action( 'gform_after_submission', array('GFMarketo', 'post_to_third_party'), 10, 2 );
        }
     ...
     * @param array $form Current GF form array
     */
    static function add_merge_tags($form){
    ?>
    <script>
...
<?php
        ...
        // if(!$soap) {
        // @start ENABLE REST THROUGH CURL
        // Make sure server setup has PHP CURL
        $curl_enabled = self::check_curl();
        if(!$curl_enabled){
?>
        <h2><?php _e('CURL Required', 'gravity-forms-marketo'); ?></h2>
        <p style="font-size:1.2em"><?php _e('This plugin requires your server to have CURL installed and enabled. Contact your web host to have them enable CURL for your account.'); ?></p>
<?php
            return;
        }
...
?>
        ...
        <form method="post" action="<?php echo add_query_arg(array('settings-updated' => true), remove_query_arg(array('refresh', 'retrieveListNames', '_wpnonce'))); ?>">
            ...
            <table class="form-table" style="clear:none; width:auto;">
                <tr>
                    <th scope="row"><label for="gf_marketo_endpoint"><?php _e("Marketo REST Endpoint URL", "gravity-forms-marketo"); ?></label> </th>
                    <td><input type="text" id="gf_marketo_endpoint" class="text code" size="50" name="gf_marketo_endpoint" placeholder="https://062-ZJQ-081.mktorest.com/rest/v1" value="<?php esc_attr_e(@$settings["endpoint"]); ?>"/></td>
                </tr>
                <tr>
                    <th scope="row"><label for="gf_marketo_user_id"><?php _e("Marketo Client ID <span class='howto'>Access Key</span>", "gravity-forms-marketo"); ?></label> </th>
                    <td><input type="text" id="gf_marketo_user_id" class="text code" size="50"  name="gf_marketo_user_id" placeholder="ea455e5f-755e-4036-bcbf-28e896d8c535" value="<?php esc_attr_e(@$settings["user_id"]); ?>"/></td>
                </tr>
                <tr>
                    <th scope="row"><label for="gf_marketo_encryption_key"><?php _e("Marketo Secret Key <span class='howto'>Secret Key</span>", "gravity-forms-marketo"); ?></label> </th>
                ...
                <tr>
                    ...
                    <td><input type="text" id="gf_marketo_subdomain" class="text code" size="50"  name="gf_marketo_subdomain" placeholder="062-ZJQ-081" value="<?php esc_attr_e(@$settings["subdomain"]); ?>"/></td>
                </tr>
                <tr>
    ...
    public static function get_api($return_exceptions = false){
        ...
        // Setup the client
        $client_id = self::get_setting('user_id');
        $secretKey = self::get_setting('encryption_key');
        $munchkin = self::get_setting('subdomain');
        $debug = self::get_setting('debug');

        // if(!self::check_soap()) { return false; }
        if(!self::check_curl()) { return false; }
        ...
        if(empty($client_id) || empty($secretKey) || empty($munchkin)) {
            return false;
        }

        // if(!class_exists("MarketoClient"))
        // if(!class_exists("overdoseRest")){

            // Require SOAP Marketo client
            // require_once(plugin_dir_path(__FILE__)."api/Marketo_SOAP_PHP_Client.php");

            // Require REST API client (overdose custom REST class)
//            require_once(plugin_dir_path(__FILE__)."api/overdose.rest.class.php");
        // }

        try {
            // $client = new MarketoClient($accessKey, $secretKey, $soapEndPoint, $debug);
            // $client->setTimeZone(wp_get_timezone_string()); // Not needed for REST client
            $client = new overdoseRest($munchkin, $client_id, $secretKey);
    ...
    private static function test_api($echo = false) {
        ...
        if(empty($endpoint) && empty($secretKey)) {
            ...
            $message = wpautop(__('Your Secret Key is required, please <label for="gf_marketo_encryption_key"><a>enter your Secret Key below</a></label>.', 'gravity-forms-marketo'));
            $works = false;
        } elseif(empty($client_id)) {
            $message = wpautop(sprintf(__('Your Client ID is required, please %senter your Client ID below%s.', 'gravity-forms-marketo'), '<label for="gf_marketo_user_id"><a>', '</a></label>'));
            $works = false;
        } else {
            ...
            if(!empty($api) && is_a($api, 'Exception')) {
                $message = wpautop(str_replace('[', __('Error key: [', 'gravity-forms-marketo'), str_replace(']', ']<br />Error message: ', $api->last_error)));
                $works = false;
    ...
    private static function edit_page(){
        ...
        if(($api === false) || is_string($api)) {
            ?>
            <div class="error" id="message" style="margin-top:20px;"><?php echo wpautop($_SESSION['token']." ".sprintf(__("We are unable to login to Marketo with the provided username and API key.  Please make sure they are valid in the %sSettings Page%s", "gravity-forms-marketo"), "<a href='?page=gf_settings&addon=Marketo'>", "</a>")); ?></div>
            <?php
            return;
        }
...
?>
    ...
    private static function get_fields() {
        ...
        // Require Overdose REST connector Class
        require_once(plugin_dir_path(__FILE__)."api/overdose.rest.class.php");
        ...
        // Instantiate REST connector Object
        $client = new overdoseRest($munchkin, $client_id, $secretKey);

        // Get leads fields
        $client->get_lead_fields();

        foreach($client->arr_leads["leads"] as $field){
            $fields[] = "leads-$field";
        }

        //Get custom objects
        $client->get_cust_obj_names();

        foreach($client->arr_cust_obj as $obj => $arr_field){
        ...
        // The original plugin developer has just hardcoded this array
        // $fields = array(
    ...
    public static function post_to_third_party( $entry, $form ) {

        // Register wp_query object 
        global $wpdb;

        // Get Marketo table name dynamically
        $table_name = $wpdb->prefix . "rg_marketo";

        // Get ID of submitted Gravity Form
        $id = (int)$form["id"];
        ...
        // Check if the GF form submitted is actually mapped to Marketo
        if(self::is_gf_form_mapped($table_name, $id)){

            // Get meta field mapping for GF / Marketo
            $sql = $wpdb->prepare("SELECT id, form_id, is_active, meta FROM $table_name WHERE form_id=%d", $id);
            $results = $wpdb->get_results($sql, ARRAY_A);
            $result = $results[0];

            // Unserialize the mappings so they can be used
            $result["meta"] = maybe_unserialize($result["meta"]);

            // Array to sort Marketo data
            $arr_marketo_data = array();

            // Iterate through field map
            foreach($result["meta"]["field_map"] as $key => $value){

                // Explode out marketo API method and marketo field from key
            ...
            // Create standard object to format data to translate to json for REST API
            $obj_php = new StdClass();

            // json request variable
            $json_request = "";

            // load and fire the API request
            $objRest = new overdoseRest($munchkin, $client_id, $secretKey); // Reset a new API object for each request

            foreach ($arr_marketo_data as $method => $fields) {

                if($method == "CustomFields"){
                    continue;
                }

                // Create standard object to format data to translate to json for REST API (reset each iteration)
                $obj_php = new StdClass();
                $obj_php->action = "createOrUpdate";

                if($method == "leads"){
                    // leads specific API presets
                    $obj_php->lookupField = "email";
                }else{
                    // Custom objects specific API presets
                    $obj_php->dedupeBy = "dedupeFields";
                }

                // Reset inputs array
                $inputs = array();
    
                foreach ($fields as $mk => $gf){

                    // Mapping Marketo to GF fields happens here
                    $inputs[$mk] = rgar( $entry, "$gf" );

                }

                // Nest inputs array inside input array
                $input = array($inputs);

                // Assign input array to php object variable
                $obj_php->input = $input;

                if(!empty($inputs)){
                    // json encode the REST request for posting
                    $json_request = json_encode($obj_php);
                  
                    $objRest->objRequest = $json_request;
                    if($method == "leads"){

                        $objRest->apiMethod = $method; // API method
                        $objRest->REST_connect("POST", "leads");

                        if(isset($objRest->objResponse->result[0]->id) && isset($obj_php->input[0]['cookies'])){
//                            error_log(" \n ID: \n ".$objRest->objResponse->result[0]->id . " \n Cookie: \n ".$obj_php->input[0]['cookies']);
                            $objRest->associate_lead($objRest->objResponse->result[0]->id, $obj_php->input[0]['cookies']);
                        }else{
                            error_log(" \n Response: \n ".$objRest->objResponse->result[0]->id . " \n Cookie: \n ".$obj_php->input[0]['cookies']);
                        }

                    }else if($method != "CustomFields"){
                        $objRest->customObj = $method; // Custom object method
    ...
    public static function is_gf_form_mapped($table, $id) {
        global $post, $wpdb;

        $rowcount = $wpdb->get_var("SELECT COUNT(*) FROM $table WHERE form_id=$id");
