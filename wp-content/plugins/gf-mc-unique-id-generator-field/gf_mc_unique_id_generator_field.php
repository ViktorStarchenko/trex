...
License: GNU GPL v.2
*/
define ("MCGFUIDGEN_PLUGIN_VERSION","1.30");
...
function mcgfuidgen_field_standard_settings( $position, $form_id ) {
	...
	if ( $position == 20 ) {
        ...
        <li class="uidgen_length_setting field_uidgen field_setting">
            <label class="mcgfuidgen_iblock" style="margin-bottom: 0 !important;">
							<?php _e( 'Maximum length', MCGFUIDGEN_TRANSLATE_DOMAIN ); ?>
							<?php gform_tooltip( 'form_field_uidgen_length_tooltip' ) ?>
            </label>
            <select class="field_uidgen_length small gfield_select" onchange="mcgfuidgen_save_settings()">
        ...
        <li class="uidgen_chartype_setting field_uidgen field_setting">
            ...
            <select class="field_uidgen_chartype medium gfield_select" onchange="mcgfuidgen_save_settings()">
...
?>
        <li class="uidgen_sequence_setting field_uidgen field_setting">
            <input type="checkbox" class="field_sequence_enabled"  onclick="mcgfuidgen_save_settings()" />
            <label class="mcgfuidgen_iblock" style="width: 75%!important; margin-bottom: 0 !important; display: inline-block !important;">
							<?php _e( 'Sequence values', MCGFUIDGEN_TRANSLATE_DOMAIN ); ?>
							<?php gform_tooltip( 'form_field_uidgen_sequence_tooltip' ) ?>
            </label>
            <input type="number" class="field_sequence_start medium" onchange="mcgfuidgen_save_settings()" onblur="mcgfuidgen_save_settings()" placeholder="<?php _e( 'Start value', MCGFUIDGEN_TRANSLATE_DOMAIN ); ?>" min="0" step="1" />
            <input type="number" class="field_sequence_step small" onchange="mcgfuidgen_save_settings()" onblur="mcgfuidgen_save_settings()" placeholder="<?php _e( 'Step', MCGFUIDGEN_TRANSLATE_DOMAIN ); ?>"  min="1" step="1" />
        </li>
		<?php
	}
}
...
?>
     <li class="uidgen_render_setting field_uidgen field_setting">
         ...
         <select class="field_uidgen_render medium gfield_select" onchange="mcgfuidgen_save_settings()">
         ...
         <textarea class="field_uidgen_render_custom_html" placeholder="Custom HTML" style="width:  96%; margin-top: 8px;"  onchange="mcgfuidgen_save_settings()" onblur="mcgfuidgen_save_settings()"><div><?php echo MCGFUIDGEN_UNQIUEID_TAG;?></div></textarea>
     </li>
<?php
	}
}
...
function mcgfuidgen_editor_js(){
	wp_enqueue_script( 'mcgfuidgen-editor', MCGFUIDGEN_PLUGIN_URL."/assets/js/editor.js", array("jquery"), MCGFUIDGEN_PLUGIN_VERSION, true );
	wp_enqueue_script( 'mcgfuidgen-jquery-stringify', MCGFUIDGEN_PLUGIN_URL."/assets/js/jQuery.stringify.js", array("jquery"), MCGFUIDGEN_PLUGIN_VERSION, true );
...
if ( defined( "MCGFUIDGEN_DEBUG" ) ) mcgfuidgen_log(">mcgfuidgen_load_value post_id = $post_id");
    $sql = $wpdb->prepare("SELECT id_int, id_str FROM `".MCGFUIDGEN_TABLE_NAME."` WHERE post_id = %s", array($post_id));
...
function mcgfuidgen_field_input($input, $field, $value, $entry_id, $form_id){
	if ($field->type == 'uidgen') {
		$post_id = (int)@$GLOBALS["mcgfuidgen_post_id"];
